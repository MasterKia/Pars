...
local function get_description(msg, data)
  ...
  if not data[tostring(msg.to.id)][data_cat] then
    return 'هیچ توضیحی برای گروه در دسترس نیست'
  end
...
local function lock_group_arabic(msg, data, target)
  ...
  if group_arabic_lock == 'yes' then
    return 'چت به زبان فارسی از قبل قفل شده است'
  else
...
local function unlock_group_arabic(msg, data, target)
  ...
  if group_arabic_lock == 'no' then
    return 'قفل چت به زبان فارسی از قبل باز است'
  else
    data[tostring(target)]['settings']['lock_arabic'] = 'no'
    save_data(_config.moderation.data, data)
    return 'قفل چت به زبان فارسی باز شد '
  end
end
...
local function lock_group_membermod(msg, data, target)
  ...
  if group_member_lock == 'yes' then
    return 'قفل ادد کردن از قبل فعال است'
  else
    data[tostring(target)]['settings']['lock_member'] = 'yes'
    save_data(_config.moderation.data, data)
  end
  return 'قفل ادد کردن فعال شد'
end
  ...
  --[[if matches[1] == 'public' then
    ...
    if matches[1] == 'newlink' and not is_realm(msg) then
      ...
      local function callback (extra , success, result)
        ...
        if success == 0 then
           return send_large_msg(receiver, 'چون ربات ، سازنده گروه نیست امکان ساخت لینک وجود ندارد !')
        end
    ...
    if matches[1] == 'link' then
      ...
      if not group_link then 
        return "ابتدا با /newlink یک لینک جدید بسازید !"
      end
